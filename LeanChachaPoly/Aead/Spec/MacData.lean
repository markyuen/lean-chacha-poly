...
-- NOTE: these two injectivity lemmas are still unproved. They are used only
-- for security analysis (not the correctness roundtrip), and the original
-- `simp [List.append_inj]` shape no longer holds — a real proof must first
-- derive the length equality before applying append injectivity.
