...
/-- XOR a byte with itself is zero. -/
@[simp]
theorem UInt8.xor_self (x : UInt8) : x ^^^ x = 0 := by decide

/-- XOR with zero is identity. -/
@[simp]
theorem UInt8.xor_zero (x : UInt8) : x ^^^ 0 = x := by decide

/-- XOR is its own inverse: (x ^^^ k) ^^^ k = x. -/
@[simp]
theorem UInt8.xor_cancel (x k : UInt8) : (x ^^^ k) ^^^ k = x := by
  simp [UInt8.xor_assoc, UInt8.xor_self]

/-- XOR is commutative. -/
theorem UInt8.xor_comm (x y : UInt8) : x ^^^ y = y ^^^ x := by decide

/-- XOR is associative. -/
theorem UInt8.xor_assoc' (x y z : UInt8) : (x ^^^ y) ^^^ z = x ^^^ (y ^^^ z) := by
  decide
...
theorem xorBytes_self (xs : List UInt8) :
  ...
  | nil => rfl
  | cons h t ih =>
    simp [xorBytes, List.zipWith, UInt8.xor_self, ih]

/-- xorBytes is symmetric. -/
theorem xorBytes_comm (xs ys : List UInt8) :
    xorBytes xs ys = xorBytes ys xs := by
  simp [xorBytes, List.zipWith_comm (f := (· ^^^ ·))]
  congr 1; ext x y; exact UInt8.xor_comm x y
...
theorem xorBytes_length_eq (xs ys : List UInt8) (h : xs.length = ys.length) :
  ...
  simp [xorBytes_length, h, Nat.min_self]
...
theorem xorBytes_self_cancel (msg ks : List UInt8) (h : msg.length = ks.length) :
  ...
  induction msg, ks using List.zipWith.induct with
  | nil_left  => simp [xorBytes]
  | nil_right => simp [xorBytes]
  | cons x k xs ks ih =>
    simp [xorBytes, List.zipWith, UInt8.xor_cancel]
    apply ih
    simpa using h
